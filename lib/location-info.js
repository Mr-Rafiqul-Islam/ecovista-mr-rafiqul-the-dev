...
export const getLocationLatLongList = async () => {
    try {
        const response = await fetch(`https://ecovista-mr-rafiqul-the-dev.vercel.app/api/location`);
...
export const getLocationLatLong = async (locationName) => {
    try {
        const response = await fetch(
            `https://ecovista-mr-rafiqul-the-dev.vercel.app/api/location/${locationName}`
        );
        const data = await response.json();
        return data;
    } catch (e) {
